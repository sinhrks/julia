...
#ifdef LLVM37
    Value *ret = builder.CreateCall(prepare_call(preffunc), { get_gcrooted(parg, ctx), iarg });
#else
    Value *ret = builder.CreateCall2(prepare_call(preffunc), get_gcrooted(parg, ctx), iarg);
#endif
    jl_value_t *ety;
...
#ifdef LLVM37
    builder.CreateCall(prepare_call(psetfunc), { get_gcrooted(parg, ctx), xarg, iarg });
#else
    builder.CreateCall3(prepare_call(psetfunc), get_gcrooted(parg, ctx), xarg, iarg);
#endif
    return parg;
}
